...
from ui.main_ui import CommTrackUI
from ui.station_ui import StationUI
from ui.packagemanager_ui import PackageManagerUi

# Main Application

class MainWindow(CommTrackUI):
    def __init__(self):
    ...
    def generate_map(self):
        try:
            ...
            map.get_root().html.add_child(folium.Element('<script src="map_functions.js"></script>'))
    ...
    def load_drawings(self, map):
        try:
            if os.path.exists('drawings.geojson'):
                with open('drawings.geojson', 'r') as f:
                    geojson_data = json.load(f)
                    folium.GeoJson(geojson_data).add_to(map)
        except Exception as e:
            print(f"Error loading drawings: {e}")

    def connect_js_methods(self):
        self.map_view.page().runJavaScript("window.pyqt_save_drawings = function(data) { pyqt_save_drawings(data); }")

    def save_drawings(self, geojson_data):
        try:
            with open('drawings.geojson', 'w') as f:
    ...
    def save_map(self):
        self.map_view.page().runJavaScript("saveDrawings();")
    ...
    def open_packagemanager_ui(self):
        self.packagemanager_ui = PackageManagerUi()
        self.packagemanager_ui.show()
